...
<van-swipe :loop="false" :width="135" :show-indicators="false">
  ...
  <i class="iconfont icon-gouwuche f-c-b"></i>
  </div>
   <div class="title3" @click="details(item,index)">查看详情</div>
  </div>
  </div>
  </van-swipe-item>
...
<script>
 export default {
   ...
   methods: {
    details(item,index){
      this.$router.push({name:'/details',query:{id:recommend[index].goodsId}})
