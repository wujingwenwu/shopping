  ...
  <div>
    ...
    <div v-else-if="nickname !=null"> 
      ...
      <div class="d-f j-c-s-a m-t-20">
        ...
        <div>
          <van-icon name="records"  size="30px"  :badge="num"/>
          <div @click="clickitemo">评价</div>
        </div>
        <div>
          <van-icon name="points" size="30px"  @click="clickmine" :badge="list.length+1"/>
          <div>已完成</div>
        </div>
      </div>
      <div class="iten"></div>
      ...
      <van-cell title="收藏商品" is-link icon="points" to="Collection" />

      <van-cell title="地址管理" is-link icon="cash-back-record" to="List" />

      <van-cell title="最近游览" is-link icon="gift-o" to="Visit" />
    </div>
  </div>
</template>
    ...
    this.$api
      ...
      .catch(err => {
       ...
       this.$api.myOrder().then(res=>{
         res.numList.map(item => {
           if (item > 0) {
             this.num = item
           }
         })
        this.list=res.numList
         console.log(res);
       }).catch(err=>{
         console.log(err);
       })
