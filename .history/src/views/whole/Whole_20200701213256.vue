  ...
  <van-tab title="已评价">
    ...
    <div v-for="(item1,index1) in item.goods" :key="index1">
     <div class="d-f m-t-20">
      <img :src="item1.image" alt="" width="80">
      <div>{{item1.name}}</div>
      <div class="whole3 d-f j-c-c a-i-c" @click="wholesa(item)">
     <van-icon name="chat" color="red" />
     <div class=" f-c-ho f-s-14">评论详情</div>
      </div>
     </div>
    </div>
    </div>
...
<script>
 export default {
   ...
   methods: {
     ...
     wholesa(item){
         this.$router.push({name:'Articulars',query:{items:item}})
     }
   },
   mounted() {
         this.$api
