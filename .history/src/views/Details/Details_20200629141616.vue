  ...
  <div>
    ...
    <div>
      ...
      <div class="d-f j-c-s-a m-t-15">
        ...
        <div>
         <div v-if="flag===false &&  this.cisCollection===1" @click="collection">
          点击收藏
          <span class="collection">
            <van-icon name="like-o" color="red"/>
          </span>
        </div>
        <div v-else-if="flag===true &&  this.cisCollection===0"  @click="collections">
        取消收藏
          <span class="collection">
            <van-icon name="like" color="red"/>
          </span>
        </div>
         <div v-else-if="flag===true"  @click="collections">
        取消收藏
          <span class="collection">
            <van-icon name="like" color="red"/>
          </span>
...
export default {
  ...
  methods: {
    ...
    collections(){
      this.flag=false,
     this.$api.cancelCollection(this.idz).then(res=>{
       localStorage.setItem("id", this.idz); 
       console.log(res);
     }).catch(err=>{
       console.log(err);
     })
  ...
  mounted() {
    ...
    this.$api
      ...
      .then(res => {
        ...
        this.idz = res.goods.goodsOne.id;
        console.log(res);
      })
      .catch(err => {
        console.log(err);
