...
export default {
  ...
  methods: {
    getData() {
      this.$api
        ...
        .then(res => {
          ...
          let asd =this.shopList.length
          localStorage.setItem('asd',this.shopList.length)
          console.log(res);
    ...
    // 结算页面
    Goto() {
      this.msg = this.shopList.filter(item1 => {
        return item1.check === true;
      });
      this.$router.push({name:'Settlements',query:{msg:this.msg,flagy:this.flagy,total:this.total}});
  ...
  mounted() {
    ...
    this.value=this.$route.query.count;
