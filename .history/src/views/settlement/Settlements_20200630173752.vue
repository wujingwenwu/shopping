    ...
    <div v-else-if="this.flagy===false">
      ...
      <van-contact-card type="cardType" :name="list.name" :tel="list.tel" @click="showList" />
       <div class="d-f">
        ...
        <div class="count">X{{this.count}}</div>
      </div>
      <div>
        <van-submit-bar :price="totals*100" button-text="提交订单" @submit="onSubmits" />
      </div>
    </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      onSubmits() {
      this.msgz= this.arrs.push(this.msga.id);
      this.totalq=this.totals
      this.$api
        .order({
          address: this.list.address,
  ...
  computed: {
     totals(){
      return this.msga.present_price*this.count;
     }  
